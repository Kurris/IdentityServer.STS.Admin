						...
						<el-form ref="client" label-position="left" :model="form" label-width="240px">
							...
							<el-form-item label="允许跨域来源">
								<template v-for="item in form.allowedCorsOrigins">
									<div :key="item.origin">
										<el-tag closable @close="form.allowedCorsOrigins = form.allowedCorsOrigins.filter(x => x.origin != item.origin)"> {{ item.origin }}</el-tag>
									</div>
								</template>
							...
							<el-form-item label="授权后重定向Uri">
								<template v-for="item in form.redirectUris">
									<div :key="item.redirectUri">
										<el-tag closable @close="form.redirectUris = form.redirectUris.filter(x => x.redirectUri != item.redirectUri)"> {{ item.redirectUri }}</el-tag>
									</div>
								</template>
							...
							<el-form-item label="注销后重定向Uri">
								<template v-for="item in form.postLogoutRedirectUris">
									<div :key="item.postLogoutRedirectUri">
										<el-tag closable @close="form.postLogoutRedirectUris = form.postLogoutRedirectUris.filter(x => x.postLogoutRedirectUri != item.postLogoutRedirectUri)">
											{{ item.postLogoutRedirectUri }}
										</el-tag>
									</div>
								</template>
								<el-input v-model="postLogoutRedirectUri">
									<el-button slot="append" type="primary" @click="addPostLogoutRedirectUri"> 添加注销后重定向Uri </el-button>
								</el-input>
							</el-form-item>
						</el-form>
					</el-card>
							...
							<el-tab-pane label="基本" name="claims">
								...
								<el-form-item label="允许的作用域">
									<template v-for="item in form.allowedScopes">
										<div :key="item.scope">
											<el-tag closable @close="form.allowedScopes = form.allowedScopes.filter(x => x.scope != item.scope)"> {{ item.scope }}</el-tag>
										</div>
									</template>
									<div>
										<el-select v-model="scope" v-if="scopes.length > 0 && form.allowedScopes">
											<el-option v-for="(item, index) in scopes.filter(x => form.allowedScopes.map(x => x.scope).indexOf(x) < 0)" :key="index" :value="item" :label="item">
											</el-option>
										</el-select>
										<el-button style="margin-left: 5px" type="primary" @click="addScope">添加作用域 </el-button>
									</div>
								</el-form-item>
								<el-form-item label="允许的授权类型" v-if="grantTypes.length > 0 && form.allowedGrantTypes">
									<template v-for="item in form.allowedGrantTypes">
										<div :key="item.grantType">
											<el-tag closable @close="form.allowedGrantTypes = form.allowedGrantTypes.filter(x => x.grantType != item.grantType)"> {{ item.grantType }}</el-tag>
										</div>
									</template>
									<div>
										...
										<el-button style="margin-left: 5px" type="primary" @click="addGrantType">添加授权类型 </el-button>
									</div>
								</el-form-item>
								...
								<el-form-item v-if="form.identityProviderRestrictions">
									...
									<template v-for="item in form.identityProviderRestrictions">
										<div :key="item.provider">
											<el-tag closable @close="form.identityProviderRestrictions = form.identityProviderRestrictions.filter(x => x.provider != item.provider)">
									...
									<div>
										<el-select v-model="identityProviderRestriction">
											<el-option
												...
												:key="item.authenticationScheme"
												:value="item.authenticationScheme"
												:label="item.displayName"
											>
											</el-option>
										</el-select>
										<el-button style="margin-left: 5px" type="primary" @click="addIdentityProviderRestriction">添加外部登录限制 </el-button>
									</div>
								</el-form-item>
			...
			<div style="display: flex; align-items: center; justify-content: space-around">
				<div style="display: flex; align-items: center; justify-content: space-around">
					...
					<el-select v-model="hashType" placeholder="请选择" v-if="enums != null">
						<el-option v-for="item in enums.hashType" :key="item.id" :label="item.text" :value="item.id"> </el-option>
					</el-select>
				</div>
				<el-button type="primary" @click="generateSecret">生成新的应用密钥</el-button>
			</div>
			<el-alert v-if="secret != null" :title="secret" description="确保您已经复制当前的密钥,这将不会再次显示" type="warning" show-icon :closable="false"> </el-alert>
			<el-divider></el-divider>

			<div v-loading="secretLoading">
				<template v-for="item in clientSecrets">
					<AuthorizeItem :key="item.id" divider :title="'secret: ' + item.description" :description="'创建时间: ' + item.created">
						<template slot="img">
