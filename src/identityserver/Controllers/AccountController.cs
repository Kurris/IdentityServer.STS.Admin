...
public class AccountController : ControllerBase
{
    ...
    public async Task<ApiResult<object>> CheckLoginAndGetUiSetting(string returnUrl)
    {
        ...
        _logger.LogDebug("Enabled external login providers: {Providers}", string.Join(",", output.ExternalProviders.Select(x => x.DisplayName)));
    ...
    private async Task<LoggedOutOutput> BuildLoggedOutModelAsync(string logoutId)
    {
        ...
        {
            PostLogoutRedirectUri = context?.PostLogoutRedirectUri,//前端配置需要和数据库保持一致,否则null
    ...
    private async Task<LoginOutput> BuildLoginResultAsync(string returnUrl)
    {
        ...
        if (context?.Client.ClientId != null)
        {
            ...
            if (client != null)
            {
                ...
                {
                    externalProviders = externalProviders.Where(provider => client.IdentityProviderRestrictions.Contains(provider.AuthenticationScheme));
                }
            }
        }
        ...
        return new LoginOutput
        {
            ...
            ReturnUrl = returnUrl,
            UserName = context?.LoginHint,
            ExternalProviders = externalProviders.OrderBy(x => x.AuthenticationScheme)
        };
    }
